...
商品编辑页同时提供“规则分类”和“AI 分类”。规则分类使用 1688 来源类目映射、标准类目别名和 `category_classification_rules`；AI 分类复用“配置 → Magic AI → 平台”，只发送商品摘要及本地预筛选后的有限候选，返回 ID 必须属于候选集。
...
Products 列表只展示可配置主商品，并显示当前商品类目及分类类型（规则分类、AI 分类、手动分类或未分类）。
