...
- `PRODUCT_INFO_MANAGEMENT_CONNECT_TIMEOUT`：建立连接的最长秒数。
- `PRODUCT_INFO_MANAGEMENT_ACCEPTANCE_TIMEOUT`：Web 请求等待任务受理的最长秒数。
- `PRODUCT_INFO_MANAGEMENT_WORKER_TIMEOUT`：后台翻译调用的最长秒数。
- `PRODUCT_INFO_MANAGEMENT_DOWNLOAD_TIMEOUT`：保存译图时下载文件的最长秒数。
